...
namespace Prosoft.Company.Fixtures;
public class DatabaseFixture : IDisposable
{
    ...
    public DatabaseFixture()
    {
        ...
        _connectionFactory = new Repositories.DbConnectionFactory(_configManager);
        SetupEnvironment();
    }

    public EmpresasModel CreateModel()
    {
        var faker = new Faker<EmpresasModel>("pt_BR")
    ...
    public void AddModel(EmpresasModel model)
    {
    ...
    private void SetupEnvironment()
    {
        ...
        using (var connection = _connectionFactory.CreateConnection())
        {
            ...
            if (File.Exists(createTableScriptPath))
            {
                connection.Execute(File.ReadAllText(createTableScriptPath));
            }
            else
            {
                Console.WriteLine($"Arquivo de script {createTableScriptPath} não encontrado.");
            }
        }
    }
    ...
    private void RemoveEnvironment()
    {
        using (var connection = _connectionFactory.CreateConnectionTest())
        {
            try
            {
                ...
                System.Threading.Thread.Sleep(1000); // 1 segundo (ajuste conforme necessário)
